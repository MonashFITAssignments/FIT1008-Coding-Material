...
def check_heap_ordering(heap, ordering):
    heap_array = heap._ProtectedAbstractHeap__UnorderedArrayHeap__array
    bound = len(heap)
...
class TestLinkedHeap(TestCase):
    ...
    def test_add(self):
        ...
        for i in range(num):
            lh.add(i)
            tree = lh._ProtectedAbstractHeap__MinLinkedHeap__root
            rank = tree.key
            for _ in range(rank):
                tree = tree.right
    ...
    def test_unbalanced(self):
        ...
        for i in range(20, 0, -1):
            lh.add(i)
            self.assertEqual(lh._ProtectedAbstractHeap__MinLinkedHeap__root.key, 1)
